...
FE_thumbnail <- ggplot(data = data.frame(x = c(0,1),
                         y = c(0,1)),
       aes(x = x, y = y)) +
  geom_point(color = "transparent") +
  geom_vline(xintercept = .5, lty = 2, size = 1, color = "darkgrey") +
  geom_hline(yintercept = .5, lty = 2, size = 1, color = "darkgrey") +
  ...
  labs(x = "Thermal tolerance", y = "Thermal plasticity")

FE_thumbnail

ggsave(FE_thumbnail,
       units = "in", width = 6, height = 6, 
       filename = "~/Documents/GitHub/Plasticity_Selection/Figures/FE_thumbnail.png",
       bg = "transparent")
...
Read in and wrangle fecundity, body size, LT50, and LT50 plasticity
...
# Create season-specific sibships IDs
fec_meta_df$Sibship <- paste(fec_meta_df$Season,
                            fec_meta_df$Sibship,
                            sep = "_")

# Read in LT50 data set from F3_LT50.rmd
...
# Filter fecundity data for observations within 7 days of LT50 assay 
...
How many sibships were derived from replacement cultures following a maternal death?

```{r}

count_ends_with_letter <- sum(str_detect(unique(sibship_LT50_filt$Sibship), "[a-zA-Z]$"))

count_ends_with_letter / nrow(as.data.frame(unique(sibship_LT50_filt$Sibship)))
...
# Scale continuous variables for model
...
# What sibships have an LT50 measure in one temp but no pl between temps?
check_df <- filter(sel_grad_df_filt, Temp == 16.42)
check_df$Diff <- check_df$LT50 - check_df$LT50_lo # RMR1_A1 and SC4_B2

# Correct for non-independence between tolerance intercept and slope using orthogonal polynomials
...
Fit and select Bayesian selection gradient models

```{r}

# brm multivariate selection gradient models
bf1a <- bf(Scaled_egg ~ Season + Scaled_LT50 + Scaled_Q10 + 
...
# Create variables for of age  andsize in selection model
sel_grad_w_age$Scaled_LT50_age <- scale(sel_grad_w_age$LT50_age)
sel_grad_w_age$Scaled_pop_size <- scale(sel_grad_w_age$Pop_size)

```

# Non-linear physiological limit effect

```{r}

# Create variable for maternal length
...
posterior_interval(brm_nl_tradelim3)
...
# What was the selection gradient acting on plasticity in the top tercile of baseline tolerance?
...
# What was the selection gradient on plasticity?
summary(lm(Estimate.Scaledegg ~ Scaled_Q10, 
           data = p_sel_grad_df_filt2))

# What was the low/hi temp selection gradient on LT50?
...
Supplemental reports of multivariate selection gradient model

```{r}

# Prob of direction
post_int <- summary(brm_nl_tradelim3)

print(post_int, digits = 4)

# posterior predictive checks
pp_check(brm_nl_tradelim3, res = "Scaledegg")

pp_check(brm_nl_tradelim3, res = "ScaledQ10trans")

# Leave one out psis plot
plot(loo(brm_nl_tradelim3))
...
# Plot
data_subset <- rand_eff_test[, c("Estimate.ScaledQ10trans_Intercept", "Estimate.ScaledLT50int_Intercept")]

Fig_6B <- ggplot(data = data_subset, 
       aes(y = `Estimate.ScaledQ10trans_Intercept`, 
       x = `Estimate.ScaledLT50int_Intercept`)) +
  geom_point() +
  geom_smooth(method = "lm") +
  theme_classic(base_size = 20) +
  labs(y = expression(italic(Z)~"(Q"[10]~"LT"[50]*")"), 
       x = expression(italic(Z)~"(basal"~"LT"[50]*")"))

## Bootstrapping test of covariance
library(boot)

# Define the function to calculate covariance while controlling for VF
covariance_with_control <- function(data, indices) {
  subset_data <- data[indices, ]
  cov(subset_data$`Estimate.ScaledQ10trans_Intercept`, 
      subset_data$`Estimate.ScaledLT50int_Intercept`)
}

observed_covariance <- covariance_with_control(data_subset, 1:nrow(data_subset))

# Apply function
boot_result <- boot(data_subset, 
                    covariance_with_control, 
                    R = 10000)

library(ggridges)

Fig_6C <- ggplot(data = rbind(data.frame(t = boot_result$t, ridge = "Poly")),
       aes(x = t, y = ridge, fill = factor(stat(quantile)))) +
  geom_vline(xintercept = 0, lty = 2) +
  stat_density_ridges(
    geom = "density_ridges_gradient",
    calc_ecdf = TRUE,
    quantiles = c(0.025, 0.9725),
    alpha = 1) + 
  scale_fill_manual(
    name = "Probability", values = c("#FF0000A0", "skyblue", "#FF0000A0"),
    labels = c("<2.5%", "2.5-97.25%", ">97.25%")) +
  theme_classic(base_size = 20) +
  theme(legend.position = "none",
        axis.text.y = element_blank(),
        axis.ticks.y = element_blank()) +
  labs(x = expression(italic(COV)[italic(Z)]), y = "Density")
 
Fig_6C

# Arrange Fig_6 panel
Fig_6BC <- ggarrange(Fig_6B, Fig_6C, 
                   labels = c("B", "C"),
                   heights = c(1, 1),
...
Fig_6panel <- ggarrange(Fig6, Fig_6BC, 
                   labels = c("A", ""),
                   widths = c(1, 1),
                   ncol = 2, nrow = 1)
...
ggsave(Fig_6panel,
       units = "in", width = 12, height = 6, 
       filename = "~/Documents/GitHub/Plasticity_Selection/Figures/Fig_6panel.png",
       bg = "white")

```

Continue supplemental analyses/plots
...
## Plot mean variables (age, size, and density,) across population for supplement
# Age w/ mean +- SD
ggplot(data = LT50_age_sum,
       aes(x = Pop, y = LT50_age)) +
  geom_point(size = 3) +
...
# Does age at LT50 bear a significant effect? No
...
# Create leverage plot for supplement
Fig_S5B <- plot(loo(brm0))
Fig_S5B

```

Model selection for selection gradient + supplementary body size fig - for supplement

```{r}

bf1_age <- bf(Scaled_egg ~ Season + Scaled_LT50_age + Scaled_LT50 + Scaled_Q10 + 
                Scaled_LT50:Temp + Scaled_Q10:Temp + 
            Scaled_Q10:Scaled_LT50_int + Scaled_Q10:Scaled_LT50_int:Temp +
            (1|Pop/Season/Pool/Sibship))

bf1_size <- bf(Scaled_egg ~ Season + Scaled_pop_size + Scaled_LT50 + Scaled_Q10 + 
                Scaled_LT50:Temp + Scaled_Q10:Temp + 
            Scaled_Q10:Scaled_LT50_int + Scaled_Q10:Scaled_LT50_int:Temp +
            (1|Pop/Season/Pool/Sibship))

bf1_size_age <- bf(Scaled_egg ~ Season + Scaled_LT50_age + Scaled_pop_size + Scaled_LT50 + Scaled_Q10 + 
                Scaled_LT50:Temp + Scaled_Q10:Temp + 
            Scaled_Q10:Scaled_LT50_int + Scaled_Q10:Scaled_LT50_int:Temp +
            (1|Pop/Season/Pool/Sibship))

bf1_len <- bf(Scaled_egg ~ Season + Scaled_len + Scaled_LT50 + Scaled_Q10 + 
                Scaled_LT50:Temp + Scaled_Q10:Temp + 
            Scaled_Q10:Scaled_LT50_int + Scaled_Q10:Scaled_LT50_int:Temp +
            (1|Pop/Season/Pool/Sibship))

bf1_len_age <- bf(Scaled_egg ~ Season + Scaled_len + Scaled_LT50_age + Scaled_LT50 + Scaled_Q10 + 
                Scaled_LT50:Temp + Scaled_Q10:Temp + 
            Scaled_Q10:Scaled_LT50_int + Scaled_Q10:Scaled_LT50_int:Temp +
            (1|Pop/Season/Pool/Sibship))

bf1_len_size <- bf(Scaled_egg ~ Season + Scaled_len + Scaled_pop_size + Scaled_LT50 + Scaled_Q10 + 
                Scaled_LT50:Temp + Scaled_Q10:Temp + 
            Scaled_Q10:Scaled_LT50_int + Scaled_Q10:Scaled_LT50_int:Temp +
            (1|Pop/Season/Pool/Sibship))

bf1_len_age_size <- bf(Scaled_egg ~ Season + Scaled_len + Scaled_LT50_age + Scaled_pop_size + Scaled_LT50 + Scaled_Q10 + 
                Scaled_LT50:Temp + Scaled_Q10:Temp + 
            Scaled_Q10:Scaled_LT50_int + Scaled_Q10:Scaled_LT50_int:Temp +
            (1|Pop/Season/Pool/Sibship))
...
Fig_S12_df <- data.frame(Model = c("Base", "Base + density", "Base + age", "Base + length", 
                    ...
                    BF = c(16.18, 4.15, 4.19, 104.90, 1, 22.54, 22.17, 5.35))
