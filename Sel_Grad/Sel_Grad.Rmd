...
p_sel_grad_df_filt2 <- fitted(brm_len,
                              newdata = sel_grad_w_age_len) %>%
  as_tibble() %>%
  bind_cols(sel_grad_w_age_len)
...
# Export Fig 5 as png
png("~/Documents/GitHub/HotOnes_Tigriopus/Figures/Fig_5.png", units = "in", width = 12, 
    height = 12, 
    res = 600)

Fig_5

bf_df2 <- data.frame(Model = c("Tradeoff + Limit", "Limit", "Tradeoff", "Null"),
                    BF = c(11.37, 1.74, 6.74, 1))
...
png("~/Documents/GitHub/HotOnes_Tigriopus/Figures/Fig_6.png", units = "in", width = 7, 
    height = 7, 
    res = 600)
...
bf_pl <- bf(Scaled_Q10 ~ (1|Pop/Season/Pool/Sibship))
bf_tol <- bf(Scaled_LT50_int ~ (1|Pop/Season/Pool/Sibship))

# As test, fit model with just random effect and multivariate outcomes pl + tol
brm_len_corr <- brm(bf_pl + bf_tol + set_rescor(FALSE),
              data = sel_grad_w_age_len,
...
names(coef(brm_len_corr))

# Calculate covariance in random effect on multivariate outcomes
rand_eff_test <- ranef(brm_len_corr)
  
# Extract the conditional effect for the random intercept
rand_eff_x <- as.data.frame(rand_eff_test$`Pop:Season:Pool:Sibship`)
  
# Extract the covariance matrix between random effects on pl and tol
cov_pl_tol <- cov(rand_eff_x$Estimate.ScaledQ10trans_Intercept,
                  rand_eff_x$Estimate.ScaledLT50int_Intercept)

# Standard error in cov estimate - standard error does not overlap 0
se_cov_yz <- sqrt(var(rand_eff_x[, 1]) / nrow(rand_eff_x) * (1 + 2 * cov_pl_tol))
se_cov_yz
